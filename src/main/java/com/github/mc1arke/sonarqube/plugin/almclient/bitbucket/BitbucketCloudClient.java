...
class BitbucketCloudClient implements BitbucketClient {
    ...
    private final BitbucketCloudConfiguration bitbucketCloudConfiguration;

    BitbucketCloudClient(BitbucketCloudConfiguration config, ObjectMapper objectMapper, OkHttpClient.Builder baseClientBuilder) {
        this(objectMapper, createAuthorisingClient(baseClientBuilder, negotiateBearerToken(config, objectMapper, baseClientBuilder.build())), config);
    }

    BitbucketCloudClient(ObjectMapper objectMapper, OkHttpClient okHttpClient, BitbucketCloudConfiguration bitbucketCloudConfiguration) {
        this.objectMapper = objectMapper;
        this.okHttpClient = okHttpClient;
        this.bitbucketCloudConfiguration = bitbucketCloudConfiguration;
    }
    ...
    public void uploadAnnotations(String commit, Set<CodeInsightsAnnotation> baseAnnotations) throws IOException {
        ...
        Request req = new Request.Builder()
                ...
                .url(format("https://api.bitbucket.org/2.0/repositories/%s/%s/commit/%s/reports/%s/annotations", bitbucketCloudConfiguration.getProject(), bitbucketCloudConfiguration.getRepository(), commit, REPORT_KEY))
                .build();
    ...
    public void uploadReport(String commit, CodeInsightsReport codeInsightReport) throws IOException {
        ...
        String targetUrl = format("https://api.bitbucket.org/2.0/repositories/%s/%s/commit/%s/reports/%s", bitbucketCloudConfiguration.getProject(), bitbucketCloudConfiguration.getRepository(), commit, REPORT_KEY);
    ...
    public Repository retrieveRepository() throws IOException {
        Request req = new Request.Builder()
                .get()
                .url(format("https://api.bitbucket.org/2.0/repositories/%s/%s", bitbucketCloudConfiguration.getProject(), bitbucketCloudConfiguration.getRepository()))
                .build();
        try (Response response = okHttpClient.newCall(req).execute()) {
            validate(response);
    ...
    void deleteExistingReport(String commit) throws IOException {
        Request req = new Request.Builder()
                .delete()
                .url(format("https://api.bitbucket.org/2.0/repositories/%s/%s/commit/%s/reports/%s", bitbucketCloudConfiguration.getProject(), bitbucketCloudConfiguration.getRepository(), commit, REPORT_KEY))
                .build();
